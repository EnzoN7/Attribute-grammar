...
public abstract class AbstractArray implements Expression {
	...
	public Type getType() {
		if (! (this.array.getType() instanceof ArrayType)) {
			Logger.error("Erreur dans getType() d'AbstractArray: array de mauvaise instance dans: "+this.toString()); 
		} else if (this.index.getType() != AtomicType.IntegerType) {
			Logger.error("Erreur dans getType d'AbstractArray: index non valide dans: "+this.toString());
		}
		System.out.println("abstractarray gettype: "+this.array.getType());
